...
for INPUT_DIR in "${INPUT_DIRS[@]}"; do
    ...
    subbase_name=$(basename "$INPUT_DIR")
    
    # 2. Definir o diretório de SAÍDA específico para a base atual
    OUTPUT_DIR="$ROOT_OUTPUT_DIR/$base_name/$subbase_name"

    
    # Criar o diretório de resultados, se não existir
    mkdir -p "$OUTPUT_DIR"
    echo "========================================"
    echo "🟢 INICIANDO PROCESSAMENTO PARA A BASE: $subbase_name"
    ...
    for entry in "${sorted_files[@]}"; do
        ...
        if [ -f "$output_file" ]; then
            echo "[INFO] Resultado já existe para o grafo: $graph_name na base $subbase_name. Pulando..."
        ...
        echo "[INFO] Processando o grafo: $graph_name na base $subbase_name"
    ...
    echo "========================================"
    echo "✅ FIM DO PROCESSAMENTO PARA A BASE: $subbase_name"
